...
from testing.grabscreen import GrabScreen
import time
    ...
    def _get_frame(self):
        ...
        while True:
            if self.stop == True:
                ...
                break
            # Lower the float for higher FPS; however if too low duplicates will occur
            if time.time() - self.render_last_time > 0.001:
                self.image = self.printscreen.get_frame()
                self.render_frame_times.append(time.time() - self.render_last_time)
                self.render_frame_times = self.render_frame_times[-20:]
                self.render_last_time = time.time()
